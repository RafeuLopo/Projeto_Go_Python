...
func saveToCSV(data []ContractData, outputFilePath string) error {
	...
	for _, record := range data {
		recordRow := []string{
			...
			nullableToString(record.VlrInvestimento),
			nullableToString(record.VlrRepasse),
			nullableToString(record.VlrContrapartida),
			nullableToString(record.VlrEmpenhado),
			nullableToString(record.VlrDesembolsado),
			nullableToString(record.VlrDesbloqueado),
			nullableToString(record.VlrPago),
			nullableToString(record.VlrTaxaAdm),
